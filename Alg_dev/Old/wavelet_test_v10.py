...
def get_swa_coeff_array(img_coeffs_array, tc, perturbation='none'):
	...
	if perturbation == 'none':
		...
		for i in range(len(tc)):
			...
			temp[tc[i][0]][tc[i][1]][tc[i][2]] = temp[tc[i][0]][tc[i][1]][tc[i][2]]*(10*np.random.uniform(0,1)+0.1)
			print(temp[tc[i][0]][tc[i][1]][tc[i][2]])
		return temp
	else:
...
img_array_mask = get_image_array_from_coeffs(np.add(img_coeffs_array_adv, -1*img_coeffs_array), img_coeffs_slices, wavelet)
...
plot_images_sbs(img, img_perturbed, 1)
