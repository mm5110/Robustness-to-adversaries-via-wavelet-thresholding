...
def calc_image_tensor_from_coeffs_tensor(coeffs, levels=1):
...
def create_adversary_nontargeted(original_img_tensor, levels, K):
    ...
    for i in range(len(tc)):
      perturbed_coeffs_var.data[tc[i][0]][tc[i][1]][tc[i][2]][tc[3]] = perturbed_coeffs_var.data[tc[i][0]][tc[i][1]][tc[i][2]][tc[3]]*(5*np.random.uniform(0,1)+0.1)

    # Create image from perturbed coefficients:
    perturbed_img_tensor = calc_image_tensor_from_coeffs_tensor(perturbed_coeffs_var.data, levels)
    ...
    return perturbed_img_pil, perturbed_img_tensor, mask_img_pil, mask_img_tensor
...
pertubed_img_pil, perturbed_img_tensor, mask_img_pil, mask_img_tensor = create_adversary_nontargeted(original_img_tensor, levels, K)

original_out, original_class = classify_image(original_img_tensor, model, labels)
perturbed_out, perturbed_class = classify_image(perturbed_img_tensor, model, labels)

print('Original image -  classification: ' + str(original_class) + ', confidence: ' + str(original_out.max()))
print('Reconstructed image -  classification: ' + str(perturbed_class) + ', confidence: ' + str(perturbed_out.max()))

plot_images_sbs(original_img_pil, mask_img_pil, pertubed_img_pil, original_class, perturbed_class, mask_image_enhance, 1)
